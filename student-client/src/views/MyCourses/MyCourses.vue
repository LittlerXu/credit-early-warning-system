  ...
  <div class="home">
    <!-- <div class="alert">
      <el-alert
        title="阅读成绩确认通知"
        type="error"
        v-show="isConfirmGraduationWarning == 0"
        :closable="false"
      >
      </el-alert>
      <el-button
...
export default {
  ...
  created() {
    ...
    // let info = JSON.parse(window.sessionStorage.getItem("info-student"));
    // let info_student = JSON.parse(JSON.stringify(info));
    // this.isConfirmGraduationWarning = info_student.isConfirmGraduationWarning;
  ...
  methods: {
    ...
    async getStudentYears() {
      ...
      this.$http
        .post(`/admin/getStudentYears?studentId=${studentId}`)
        .then((res) => {
          if (res.data.code == 1) {
    ...
    //   let { studentId } = student;
    //   let result = await this.$http.post(
    //     `/admin/getStudentPreGrades?studentId=${studentId}`
    //   );
